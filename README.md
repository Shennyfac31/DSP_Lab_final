# DSP_Lab_final
[final.py](final.py)是main，跑它的話會讓你錄十次音（錄的次數可以改for的參數決定）。但你錄的音不會存檔，會直接被拿去test，然後印出它判斷的結果（括號前面的數字：1:up, 2:down, 3:left, 4:right, 0:hit。括號後面的數字：DTW的距離）。

[record_temp.py](record_temp.py)是錄音用的。錄的音會存在[train](train/)裡面。錄的時候記得改檔名不然會被rewrite喔。

[train](train/)裡面現在已經有我錄的n0~n4了。順序由n0到n4應該是hit/up/down/left/right。
